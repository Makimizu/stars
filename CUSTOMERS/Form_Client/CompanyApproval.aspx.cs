...
namespace CUSTOMERS.Form_Client
{
    public partial class CompanyApproval : System.Web.UI.Page
    {
        ...
        protected void Setup()
        {
            ...
            conn.QueryString = "select CODE,DESCR from V_LINK_PR_CHANNEL_DISTRIBUTION";
            conn.ExecuteQuery();
            ...
            conn.QueryString = "select CODE,DESCR from PR_COMPANY_STATUS where CODE='001'";
            conn.ExecuteQuery();
            for (int i = 0; i < conn.GetRowCount(); i++)
        ...
        protected void FILL_DGR()
        {
            ...
            if (DDL_CHANNEL.SelectedValue != "")
                where = where + " and CHANEL_DIST = '" + DDL_CHANNEL.SelectedValue + "' ";
            ...
            for (int i = 0; i < DGR.Items.Count; i++)
            {
                ...
                lbCode.Text = "<a href='" + DGR.Items[i].Cells[10].Text + "'>" + DGR.Items[i].Cells[0].Text + "</a>";
            }
