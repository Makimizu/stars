...
using static REAS.Form_App.UploadTechnicalReas;

namespace REAS.Form_App
{
    public partial class UploadUpdateStatusReas : System.Web.UI.Page
    {
                ...
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                    {
                        if (DDL_TYPE.SelectedValue == "contribution_individual")
                        {
                            ...
                            decimal AmountTechnical = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 15].Text) ? "0" : worksheet.Cells[row, 15].Text);
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [AutoReportReconcile].[dbo].[SP_UploadTechnical_Individual] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
