...
{
    public partial class UploadTechnicalIndividu : System.Web.UI.Page
    {
        ...
        {
            LBL_TITLE.Text = "UPLOAD TECHNICAL INDIVIDUAL - REAS";
                    ...
                    using (var package = new ExcelPackage(new FileInfo(filePath)))
                    {
                        ...
                        for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                        {
                            if (DDL_TYPE.SelectedValue == "contribution_individual")
                            {
                                ...
                                string POLICY_NO = worksheet.Cells[row, 6].Text; // POLICY_NO
                            ...
                            else if (DDL_TYPE.SelectedValue == "claim_individual")
                            {
                                ...
                                string ID = worksheet.Cells[row, 1].Text; // ID
                                string POLICY_NO = worksheet.Cells[row, 4].Text; // POLICY_NO
                        ...
                        if (errorUpload > 0)
                        {
                            LBL_PROCESS.Text = "Terdapat <b>beberapa ID</b. yang memiliki MEMO Aktif, <br /> Apakah ingin melanjutkan process Upload Technical Lanjutan ?";
                        }
                        else
                        {
                            btnDownload.Visible = false;
                            LBL_PROCESS.Text = "Tidak ada ID yang memiliki MEMO Aktif, <br /> Apakah ingin melanjutkan process Upload Technical ?";
                        }
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            if (DDL_TYPE.SelectedValue != "")
            {
                if (DDL_TYPE.SelectedValue == "contribution_individual")
                {
                    fileName = "Upload Technical Individual Contribution.xlsx";  // Nama file yang akan diunduh
                }
                else if (DDL_TYPE.SelectedValue == "claim_individual")
                {
                    fileName = "Upload Technical Individual Claim.xlsx";  // Nama file yang akan diunduh
                }
            }
                ...
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                    {
                        if (DDL_TYPE.SelectedValue == "contribution_individual")
                        {
                            ...
                            string ID = worksheet.Cells[row, 1].Text; // ID
                            string SuratPengajuan = string.IsNullOrEmpty(worksheet.Cells[row, 11].Text) ? "" : worksheet.Cells[row, 11].Text;
                            string StatusTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 12].Text) ? "" : worksheet.Cells[row, 12].Text;
                            string NoSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 13].Text) ? "" : worksheet.Cells[row, 13].Text;
                            string PerihalSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 14].Text) ? "" : worksheet.Cells[row, 14].Text;
                            
                            decimal AmountTabbaru = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 15].Text) ? "0" : worksheet.Cells[row, 15].Text);
                            decimal AmountUjroh = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 16].Text) ? "0" : worksheet.Cells[row, 16].Text);
                            decimal AmountTechnical = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 17].Text) ? "0" : worksheet.Cells[row, 17].Text);

                            string updateDateTxt = worksheet.Cells[row, 18].Text;
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [AutoReportReconcile].[dbo].[SP_UploadTechnical_Contribution_Individual] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                        ...
                        else if (DDL_TYPE.SelectedValue == "claim_individual")
                        {
                            ...
                            string ID = worksheet.Cells[row, 1].Text; // ID
                            string SuratPengajuan = string.IsNullOrEmpty(worksheet.Cells[row, 16].Text) ? "" : worksheet.Cells[row, 16].Text;
                            string StatusTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 17].Text) ? "" : worksheet.Cells[row, 17].Text;
                            string NoSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 18].Text) ? "" : worksheet.Cells[row, 18].Text;
                            string PerihalSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 19].Text) ? "" : worksheet.Cells[row, 19].Text;
                            decimal AmountTechnical = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 20].Text) ? "0" : worksheet.Cells[row, 20].Text);
                            string updateDateTxt = worksheet.Cells[row, 21].Text;
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [AutoReportReconcile].[dbo].[SP_UploadTechnical_Claim_Individual] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                                 "'" + StatusTechnical.Trim() + "'," +
