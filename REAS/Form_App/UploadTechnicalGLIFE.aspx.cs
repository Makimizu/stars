...
{
    public partial class UploadTechnicalGLIFE : System.Web.UI.Page
    {
        ...
        {
            LBL_TITLE.Text = "UPLOAD TECHNICAL GLIFE - REAS";
        ...
        protected void BT_UPLOAD_Click(object sender, EventArgs e)
        {
            ...
            if (TXT_FILE_UPLOAD.HasFile)
            {
                try
                {
                    ...
                    string filePath = Path.Combine(Server.MapPath("~/Upload/Technical"), TXT_FILE_UPLOAD.FileName);
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                {
                    if (DDL_TYPE.SelectedValue == "claim")
                    {
                        ...
                        // Simpan ke database
                        conn.QueryString = "EXEC [dbo].[SP_RTF_CHECKDATA_GLIFE_CLAIM_JOURNAL] " +
                             "'" + ID.Trim() + "'," +
                             "'" + POLICY_NO + "'";
                        conn.ExecuteQuery();
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                {
                    if (DDL_TYPE.SelectedValue == "refund")
                    {
                        ...
                        // Simpan ke database
                        conn.QueryString = "EXEC [dbo].[SP_RTF_CHECKDATA_GLIFE_PRODUKSI_REFUND_JOURNAL] " +
                             "'" + ID.Trim() + "'," +
                             "'" + POLICY_NO + "'";
                        conn.ExecuteQuery();
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                {
                    if (DDL_TYPE.SelectedValue == "contribution_renewal")
                    {
                        ...
                        // Simpan ke database
                        conn.QueryString = "EXEC [dbo].[SP_RTF_CHECKDATA_GLIFE_PRODUKSI_RENEWAL_JOURNAL] " +
                             "'" + ID.Trim() + "'," +
                             "'" + POLICY_NO + "'";
                        conn.ExecuteQuery();
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                {
                    if (DDL_TYPE.SelectedValue == "contribution_non_gtlr")
                    {
                        ...
                        // Simpan ke database
                        conn.QueryString = "EXEC [dbo].[SP_RTF_CHECKDATA_GLIFE_PRODUKSI_GTLR_NONGTLR_JOURNAL] " +
                             "'" + ID.Trim() + "'," +
                             "'" + POLICY_NO + "'";
                        conn.ExecuteQuery();
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            if (DDL_TYPE.SelectedValue != "")
            {
                if (DDL_TYPE.SelectedValue == "contribution_gtlr")
                {
                    fileName = "Template Upload Technical Glife - Contribution GTLR.xlsx";  // Nama file yang akan diunduh
                }
                else if (DDL_TYPE.SelectedValue == "contribution_non_gtlr")
                {
                    fileName = "Template Upload Technical Glife - Contribution NON GTLR.xlsx";  // Nama file yang akan diunduh
                }
                else if (DDL_TYPE.SelectedValue == "contribution_renewal")
                {
                    fileName = "Template Upload Technical Glife - Contribution Renewal.xlsx";  // Nama file yang akan diunduh
                }
                else if (DDL_TYPE.SelectedValue == "refund")
                {
                    fileName = "Template Upload Technical Glife - Refund.xlsx";  // Nama file yang akan diunduh
                }
                else if (DDL_TYPE.SelectedValue == "claim")
                {
                    fileName = "Template Upload Technical Glife - Claim.xlsx";  // Nama file yang akan diunduh
                }
            }

            // Tentukan path folder tempat file disimpan
            string folderPath = Server.MapPath("~/Content/Template/Upload Technical GLIFE/");
                ...
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                    {
                        if (DDL_TYPE.SelectedValue == "contribution_gtlr")
                        {
                            ...
                            string ID = worksheet.Cells[row, 1].Text; // ID
                            string SuratPengajuan = string.IsNullOrEmpty(worksheet.Cells[row, 9].Text) ? "" : worksheet.Cells[row, 9].Text;
                            string StatusTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 10].Text) ? "" : worksheet.Cells[row, 10].Text;
                            string NoSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 11].Text) ? "" : worksheet.Cells[row, 11].Text;
                            string PerihalSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 12].Text) ? "" : worksheet.Cells[row, 12].Text;

                            decimal AmountTechnical = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 13].Text) ? "0" : worksheet.Cells[row, 13].Text);
                            decimal AmountTabbaru = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 14].Text) ? "0" : worksheet.Cells[row, 14].Text);
                            decimal AmountUjroh = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 15].Text) ? "0" : worksheet.Cells[row, 15].Text);

                            string updateDateTxt = worksheet.Cells[row, 16].Text;
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [dbo].[SP_RTF_UPLD_TECHNICAL_PRODUKSI_GTLR_GLIFE] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                            ...
                            if (statusDB == "FAILED")
                            {
                                LB_ERR.Text += "Data dengan ID : " + ID + ", gagal di proses membuat technical. terdapat di memo yang aktif. <BR />";
                            }
                            else
                            {
                        ...
                        else if (DDL_TYPE.SelectedValue == "contribution_non_gtlr")
                        {
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [dbo].[SP_RTF_UPLD_TECHNICAL_PRODUKSI_NONGTLR_GLIFE] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                                 "'" + StatusTechnical.Trim() + "'," +
                        ...
                        else if (DDL_TYPE.SelectedValue == "contribution_renewal")
                        {
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [dbo].[SP_RTF_UPLD_TECHNICAL_PRODUKSI_RENEWAL_GLIFE] " +
                                  "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                                 "'" + StatusTechnical.Trim() + "'," +
                        ...
                        else if (DDL_TYPE.SelectedValue == "refund")
                        {
                            ...
                            string ID = worksheet.Cells[row, 1].Text; // ID
                            string SuratPengajuan = string.IsNullOrEmpty(worksheet.Cells[row, 7].Text) ? "" : worksheet.Cells[row, 7].Text;
                            string StatusTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 8].Text) ? "" : worksheet.Cells[row, 8].Text;
                            string NoSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 9].Text) ? "" : worksheet.Cells[row, 9].Text;
                            string PerihalSuratTechnical = string.IsNullOrEmpty(worksheet.Cells[row, 10].Text) ? "" : worksheet.Cells[row, 10].Text;

                            decimal AmountTechnical = Convert.ToDecimal(string.IsNullOrEmpty(worksheet.Cells[row, 11].Text) ? "0" : worksheet.Cells[row, 11].Text);
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [dbo].[SP_RTF_UPLD_TECHNICAL_PRODUKSI_REFUND_GLIFE] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                                 "'" + StatusTechnical.Trim() + "'," +
                            ...
                            if (statusDB == "FAILED")
                            {
                                LB_ERR.Text += "Data dengan ID : " + ID + ", gagal di proses membuat technical. mohon di cek kembali data tersebut. <BR />";
                            }
                            else
                            {
                        ...
                        else if (DDL_TYPE.SelectedValue == "claim")
                        {
                            ...
                            // Simpan ke database
                            conn.QueryString = "EXEC [dbo].[SP_RTF_UPLD_TECHNICAL_CLAIMGLIFE] " +
                                 "'" + GenerateType.Trim() + "'," +
                                 "'" + ID.Trim() + "'," +
                                 "'" + SuratPengajuan.Trim() + "'," +
                                 "'" + StatusTechnical.Trim() + "'," +
